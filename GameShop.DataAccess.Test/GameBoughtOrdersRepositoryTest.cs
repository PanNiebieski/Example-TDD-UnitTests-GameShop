...
{
    public class GameBoughtOrdersRepositoryTest
    {
        [Fact]
        public void ShouldSaveTheGameBoughtOrder()
        {
            ...
            using (var context = new GameBoughtOrderContext(options))
            {
                ...
                var storedDeskBooking = orders.Single();

                Assert.Equal(gameOrder.FirstName, storedDeskBooking.FirstName);
                Assert.Equal(gameOrder.LastName, storedDeskBooking.LastName);
                Assert.Equal(gameOrder.Email, storedDeskBooking.Email);
                Assert.Equal(gameOrder.GameId, storedDeskBooking.GameId);
                Assert.Equal(gameOrder.Date, storedDeskBooking.Date);
            }
        }
